...
public:
    ...
    light_pos_(1.2f, 1.0f, 2.0f)
  {
    ...
    // Configure shader
    lighting_shader_= Shader(vertexPath, fragmentPath);
  }

  void fillTrianglesBuffer(const std::vector<Triangle> &triangles, const std::vector<size_t> &indexes) {
    std::list<size_t> index_list;
    std::copy( indexes.begin(), indexes.end(), std::back_inserter( index_list ) );
    for (size_t i = 0; i < triangles.size(); i++) {
      auto &cur_tri = triangles[i];
      if(index_list.front() == i) {
        vert_tri_red.push_back(cur_tri.getA().getX());
        vert_tri_red.push_back(cur_tri.getA().getY());
        vert_tri_red.push_back(cur_tri.getA().getZ());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getZ());
        vert_tri_red.push_back(cur_tri.getB().getX());
        vert_tri_red.push_back(cur_tri.getB().getY());
        vert_tri_red.push_back(cur_tri.getB().getZ());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getZ());
        vert_tri_red.push_back(cur_tri.getC().getX());
        vert_tri_red.push_back(cur_tri.getC().getY());
        vert_tri_red.push_back(cur_tri.getC().getZ());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_red.push_back(cur_tri.getPlane().getN().getZ());
        index_list.pop_front();
      } else {
        vert_tri_blue.push_back(cur_tri.getA().getX());
        vert_tri_blue.push_back(cur_tri.getA().getY());
        vert_tri_blue.push_back(cur_tri.getA().getZ());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getZ());
        vert_tri_blue.push_back(cur_tri.getB().getX());
        vert_tri_blue.push_back(cur_tri.getB().getY());
        vert_tri_blue.push_back(cur_tri.getB().getZ());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getZ());
        vert_tri_blue.push_back(cur_tri.getC().getX());
        vert_tri_blue.push_back(cur_tri.getC().getY());
        vert_tri_blue.push_back(cur_tri.getC().getZ());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getX());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getY());
        vert_tri_blue.push_back(cur_tri.getPlane().getN().getZ());
      }
  ...
  void renderWindow() {
    ...
    while (!glfwWindowShouldClose(window_))
    {
        ...
        glm::vec3 sunColor(1.0f, 0.96f, 0.90f); // тёплый почти белый
        ...
        glDrawArrays(GL_TRIANGLES, static_cast<size_t>((vert_tri_blue.size()-vert_tri_red.size())/6), static_cast<size_t>(vert_tri_red.size()/6));
